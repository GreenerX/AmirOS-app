...
export function FloatingAssistant({
  ...
  return <div className={`floating-assistant ${open ? "open" : ""}`}>
    {open ? <section className="floating-assistant-panel" aria-label="Ask AmirOS">
      ...
      <section className="floating-ai-history"><button onClick={() => setHistoryOpen((value) => !value)}><span><History size={14} />Answer history</span><small>{data?.questionHistory.length || 0}</small><ChevronDown size={14} className={historyOpen ? "open" : ""} /></button>{historyOpen ? <div className="floating-ai-history-body"><label><Search size={13} /><input value={historySearch} onChange={(event) => setHistorySearch(event.target.value)} placeholder="Search answers" /></label>{history.slice(0, 8).map((item) => <button key={item.id} onClick={() => { setLastQuestion(item.question); setQuery(""); setAnswer({ answer: item.answer, evidenceIds: item.sources.map((source) => source.id), sources: item.sources }); }}><span><strong>{item.question}</strong><small>{new Intl.DateTimeFormat(undefined, { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" }).format(new Date(item.createdAt))}</small></span>{pinned.has(item.question) ? <Pin size={12} /> : <ArrowRight size={12} />}</button>)}{history.length === 0 ? <p>No matching answers yet.</p> : null}</div> : null}</section>
    </section> : null}
