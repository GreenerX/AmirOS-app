      ...
      {answer ? <article className="floating-ai-answer">
        ...
        {evidenceSources.some((source) => source.explanation) ? <details className="floating-ai-memory-explanation">
          ...
          <div>{evidenceSources.filter((source) => source.explanation).slice(0, 3).map((source) => <article key={`explain-${source.id}`}>
            <strong>{source.contactName}</strong>
            <p>{source.explanation!.summary}</p>
            <small>{source.explanation!.confidenceLabel} · {source.explanation!.statusLabel} · {source.explanation!.evidenceCount} supporting {source.explanation!.evidenceCount === 1 ? "message" : "messages"}</small>
          </article>)}</div>
        </details> : null}
