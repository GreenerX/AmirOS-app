...
function AdminDashboard({ user, onNavigate, onSignOut }: { user?: ControlCenterUser; onNavigate: (page: Page) => void; onSignOut: () => void }) {
  ...
  const removeUser = () => {
    if (!selected) return;
    const confirmation = window.prompt(`Permanently remove ${selected.email} from Netlify Identity and the Control Center?\n\nThis removes account access and paired-device credentials. It never deletes AmirOS data stored locally on the Mac. Type the email address to continue.`);
    if (confirmation !== selected.email) {
      if (confirmation !== null) setApiMessage("Account removal canceled because the email did not match.");
      return;
    }
    if (demoEnabled) {
      setUsers((items) => items.filter((item) => item.id !== selected.id));
      setSelectedId(undefined);
      setApiMessage("Demo account removed.");
      return;
    }
    void deleteAdminUser({ userId: selected.id }).then((result) => {
      if (result.data?.removed) {
        setUsers((items) => items.filter((item) => item.id !== selected.id));
        setApplications((items) => items.filter((item) => item.accountId !== selected.id));
        setSelectedId(undefined);
        setApiMessage("Account removed from Netlify Identity and the Control Center. Local AmirOS data was not changed.");
      } else setApiMessage(result.message);
    });
  };
  ...
  const removeApplicantIdentity = (application: BetaApplication) => {
    const confirmation = window.prompt(`Remove the pending Identity account and invitation for ${application.email}?\n\nThis permanently removes the pending invitation. It cannot remove an active tester account; use Testers for that. Type the email address to continue.`);
    if (confirmation === null) return;
    if (confirmation.trim().toLowerCase() !== application.email.trim().toLowerCase()) {
      setApiMessage("Invitation removal canceled because the email did not match.");
      return;
    }
    if (demoEnabled) {
      setApplications((items) => items.filter((item) => item.id !== application.id));
      setApiMessage("Demo invitation removed.");
      return;
    }
    void removeBetaApplicantIdentity({ applicationId: application.id, confirmation }).then((result) => {
      if (result.data?.removed) {
        setApplications((items) => items.filter((item) => item.id !== application.id));
        setApiMessage("Pending Identity account and invitation removed.");
      } else setApiMessage(result.message);
    });
  };
...
function SelectedUser({ selected, onStatus, onProfile, onReleaseChannel, onToggleFeature, onDeviceStatus, onDelete }: { selected: AdminUser; onStatus: (status: AccessStatus) => void; onProfile: (firstName: string, lastName: string) => void; onReleaseChannel: (channel: ReleaseChannel) => void; onToggleFeature: (featureId: string) => void; onDeviceStatus: (deviceId: string, status: AccessStatus) => void; onDelete: () => void }) {
